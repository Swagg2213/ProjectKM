            ...
            <div class="space-y-4">
                ...
                <div>
                    ...
                    <select id="kategori" name="kategori" required
                        ...
                        <option value="Panitia" {{ old('kategori', $event->kategori) == 'Panitia' ? 'selected' : '' }}>Kepanitiaan</option>
                        <option value="Pengmas" {{ old('kategori', $event->kategori) == 'Pengmas' ? 'selected' : '' }}>Pengmas</option>
                        <option value="Bakmi" {{ old('kategori', $event->kategori) == 'Bakmi' ? 'selected' : '' }}>Bakat Minat</option>
                        <option value="Lainnya" {{ old('kategori', $event->kategori) == 'Lainnya' ? 'selected' : '' }}>Lainnya</option>
                    </select>
                </div>

                <div>
                    <label for="image" class="block text-sm font-medium text-gray-700" accept="image/*">Upload New Poster / Image</label>
                    <div class="mt-2">
                        <span class="block text-xs text-gray-500 mb-2">Current Image:</span>
                        <img src="{{ asset('storage/' . $event->image) }}" alt="{{ $event->title }}" class="w-48 h-auto rounded-md object-cover mb-4">
                    </div>
